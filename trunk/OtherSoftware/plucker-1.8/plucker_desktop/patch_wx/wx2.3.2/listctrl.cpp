...
// Id:          $Id: listctrl.cpp,v 1.1 2002/07/16 20:46:57 robertoconnor Exp $
...
// ROB: Set this to zero to allow zero-width columns ( but won't work )
...
{
    // ROB: A test to make sure it doesn't crash something
    wxLogDebug( "Entering wxListHeaderData::SetWidth()..." );
    m_width = w;
    if (m_width < 0)
    ...
    // ROB: A test to make sure it doesn't crash something
    wxLogDebug( "Completed wxListHeaderData::SetWidth()" );
}
...
void wxListHeaderWindow::OnPaint( wxPaintEvent &WXUNUSED(event) )
{
    ...
    for ( int i = 0; i < numColumns && x < w; i++ )
    {
        ...
        // ROB: continue to next column here if the column is zero-width
        if ( wCol == 0 ) {
            continue;
        }
        // end of ROB
