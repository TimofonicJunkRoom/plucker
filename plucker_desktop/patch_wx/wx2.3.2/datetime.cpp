...
// Modified by:
// Created:     11.05.99
// RCS-ID:      $Id: datetime.cpp,v 1.1 2002/07/16 20:46:57 robertoconnor Exp $
...
static bool GetNumericToken(size_t len, const wxChar*& p, unsigned long *number)
{
    ...
    // ROB: Patch from version 1.66 for explictly stating the base is 10
    // not an octal as 9 is not a valid octal digit
    // return !!s && s.ToULong(number);
    return !!s && s.ToULong(number, 10);
}
...
const wxChar *wxDateTime::ParseDate(const wxChar *date)
{
    ...
    while ( tok.HasMoreTokens() )
    {
        ...
        // ROB: See comment in GetNumericToken
        //if ( token.ToULong(&val) )
        if ( token.ToULong(&val,10) )
        {
