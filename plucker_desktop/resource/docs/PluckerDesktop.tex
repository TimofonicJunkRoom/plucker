...
\desktopcontrol{Execute a command before spidering this channel} executes the 
specified operating system command immediately before this channel starts its 
spider.

\desktopcontrol{Execute a command after plucking channel} executes the 
...
\begin{itemize}
  \item \configkey{autoupdate\_deferred\_lower\_time\_limit} If in Deferred
    ...
    \code{19:00:00}
  \item \configkey{autoupdate\_deferred\_upper\_time\_limit} If in Deferred
    autoupdate mode, the lower time of day limit that current time must be
    in order for an autoupdate to start. Must be in w3C format, for example:
    \code{23:00:00}
  \item \configkey{autoupdate\_mode} One of 4 autoupdate modes: 0=Never,
    1=Immediately, 2=Deferred, 3=OnSync.
  \item \configkey{autoupdate\_precision} Number of minutes to check if it is time
    to start an autoupdate, if so specified in the preferences.
  \item \configkey{username\_filter} A filter of the username to update channels.
...
\begin{itemize}
  \item \configkey{after\_command} Command to execute after spidering this channel.
  \item \configkey{before\_command} Command to execute before spidering this channel.
  \item \configkey{copy\_to\_dir} The directory to install channel's output.
  \item \configkey{directory\_on\_card}= Semicolon-delimited list of directory on handheld
